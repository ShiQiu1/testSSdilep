...
SCRIPT="./ssdilep/run/j.plotter_CF_tau.py"  # pyframe job script


JOBNAME = "testSimonTauMC"
...
def submit(tag,job_sys,samps,config={}):
  ...
  * submit job
  ...
  sample_file = './testList.txt' #list of input files
  ...
  for s in samps:
    ...
    jobName = "user.sarnling." + JOBNAME + "." + s.name + "." + (config['sys'] if len(config) > 0 else "nominal")
